...
class validate:
    ...
    def validate_entry(device_name):
    	...
    	entry= client.search(name=device_name)
    	loc_code= entry.split('_')[0]
    	if loc_code not in location_data:
    		print(f"Invalid location code: device name not found in dict. Please add new location or correct the location code")
    	else:
    			try:
    				cat_ioc_alias=entry.item.ioc_alias.split(':')[0]
    				...
    				if loc_code.lower() is not cat_name:
    					return False
    				if loc_code.lower() not in location_data:
    					return False
    				else:
    					return True
